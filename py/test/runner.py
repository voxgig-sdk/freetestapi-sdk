# Freetestapi SDK test runner

from __future__ import annotations
import os
import json

from freetestapi_sdk.utility.voxgig_struct import voxgig_struct as vs


class FreetestapiTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreetestapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreetestapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreetestapiTestRunner.getenv("FREETESTAPI_TEST_LIVE")
        override = FreetestapiTestRunner.getenv("FREETESTAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreetestapiTestRunner.getenv(key)
        ...
        explain = FreetestapiTestRunner.getenv("FREETESTAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREETESTAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreetestapiTestRunner._test_control is not None:
            return FreetestapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreetestapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreetestapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreetestapiTestRunner.load_test_control()
...
def load_env_local():
    FreetestapiTestRunner.load_env_local()


def env_override(m):
    return FreetestapiTestRunner.env_override(m)


def entity_data(v):
    return FreetestapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreetestapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreetestapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreetestapiTestRunner.load_test_control()


def live_client_options():
    return FreetestapiTestRunner.live_client_options()


def live_delay_ms():
    return FreetestapiTestRunner.live_delay_ms()
