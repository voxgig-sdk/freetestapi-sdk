...
class ProductEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = FreetestapiSDK::test($seed, null);
        $seen = iterator_to_array($base->Product(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FreetestapiConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FreetestapiSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREETESTAPI_TEST_PRODUCT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function product_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FREETESTAPI_TEST_PRODUCT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FREETESTAPI_TEST_PRODUCT_ENTID"]);
    ...
    if ($env["FREETESTAPI_TEST_LIVE"] === "TRUE") {
        ...
        $client = new FreetestapiSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["FREETESTAPI_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
