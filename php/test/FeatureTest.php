...
class FtCtrl extends FreetestapiControl
{
...
class FtClient
{
    ...
    private FreetestapiUtility $utility;

    public function __construct(FreetestapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreetestapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreetestapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreetestapiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreetestapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreetestapiConfig::make_feature($name);
    ...
    private static function buildUrl(FreetestapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreetestapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreetestapiResponse($fetched);
        }
    ...
    private function fail(FreetestapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreetestapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
