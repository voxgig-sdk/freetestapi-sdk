...
//     (\`new FreetestapiSDK(...)\` / \`FreetestapiSDK.test(...)\` become
//     \`FreetestapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"product":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreetestapiSDK(...)\` and any
// \`FreetestapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreetestapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreetestapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
