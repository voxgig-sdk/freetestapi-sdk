...
class UserEntity
  ...
  def data_set(args)
    if args
      @_data = FreetestapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FreetestapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [User, Hash] the loaded User; raises FreetestapiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreetestapiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<User>, Array] the matching User items; raises FreetestapiError on failure
