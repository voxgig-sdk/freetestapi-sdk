...
require_relative 'Freetestapi_types'


class FreetestapiSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreetestapiHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreetestapiHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreetestapiFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreetestapiUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreetestapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreetestapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreetestapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FreetestapiError.new(
        "#{op}_allow",
        "FreetestapiSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FreetestapiHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreetestapiError.new(
        "graphql_error", "FreetestapiSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FreetestapiSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
