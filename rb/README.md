# Freetestapi Ruby SDK



The Ruby SDK for the Freetestapi API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = FreetestapiSDK.test(testopts, sdkopts)
```
...
raise a `FreetestapiError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Freetestapi_sdk.rb       -- Main SDK module
...
The main module (`Freetestapi_sdk`) exports the SDK class
