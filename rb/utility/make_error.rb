...
module FreetestapiUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FreetestapiContext.new({}, nil)
    end
    op = ctx.op || FreetestapiOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FreetestapiResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FreetestapiError) ? err.msg : err.to_s
    ...
    sdk_err = FreetestapiError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FreetestapiError)
