...
class ProductEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FreetestapiSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FREETESTAPI_TEST_PRODUCT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def product_basic_setup(extra)
  ...
  entid_env_raw = ENV["FREETESTAPI_TEST_PRODUCT_ENTID"]
  ...
  if env["FREETESTAPI_TEST_LIVE"] == "TRUE"
    ...
    client = FreetestapiSDK.new(Helpers.to_map(merged_opts))
  end
