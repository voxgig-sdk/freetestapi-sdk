# Freetestapi Golang SDK



The Golang SDK for the Freetestapi API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/freetestapi-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/freetestapi-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/freetestapi-sdk/go=../freetestapi-sdk/go
```
...
```go
func NewFreetestapiSDK(options map[string]any) *FreetestapiSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *FreetestapiSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### FreetestapiSDK methods
...
| `Product` | `(data map[string]any) FreetestapiEntity` | Create a Product entity instance. |
| `User` | `(data map[string]any) FreetestapiEntity` | Create an User entity instance. |

### Entity interface (FreetestapiEntity)

All entities implement the `FreetestapiEntity` interface.
...
| `rating` | `float64` | Average product rating (0-5) |
...
```
github.com/voxgig-sdk/freetestapi-sdk/go/
├── freetestapi.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/freetestapi-sdk/go`) re-exports everything needed
