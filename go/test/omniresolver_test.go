...
import (
	...
	omni "github.com/voxgig-sdk/freetestapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreetestapiError); is {
				out := map[string]any{
					"name":    "FreetestapiError",
...
func omniCtx(arg any, client *sdk.FreetestapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreetestapiSDK); is {
			client = live
