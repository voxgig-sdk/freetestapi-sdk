...
module FreetestapiOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if FreetestapiOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = FreetestapiOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| FreetestapiOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || FreetestapiOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = FreetestapiSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = FreetestapiError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { FreetestapiOmni.errify(err) }
