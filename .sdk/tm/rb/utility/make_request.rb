...
module FreetestapiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreetestapiResponse.new({})
    ...
    if fetch_err
      ...
      response = FreetestapiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreetestapiResponse.new(fetched)
    else
