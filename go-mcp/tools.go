...
type Args struct {
	Entity string         `json:"entity" jsonschema:"product | user"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.FreetestapiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "freetestapi_list",
		Description: "List records from Freetestapi. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "freetestapi_load",
		Description: "Load a single record from Freetestapi. " +
...
func runOp(client *sdk.FreetestapiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.FreetestapiSDK, name string) (sdk.FreetestapiEntity, error) {
