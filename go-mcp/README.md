...
[MCP](https://modelcontextprotocol.io) server exposing the Freetestapi SDK as
two agent tools — `freetestapi_list` and `freetestapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/freetestapi-mcp)
make build

# 2. Provide credentials via the environment
export FREETESTAPI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user freetestapi \
  -- /absolute/path/to/freetestapi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./freetestapi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// freetestapi_list: first page of records
{ "entity": "product" }
{ "entity": "product", "query": { } }

// freetestapi_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/freetestapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user freetestapi \
     -- "$PWD"/dist/*/freetestapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `freetestapi_list` and `freetestapi_load` tools now appear
   in new sessions. Ask the agent to *"list product using freetestapi"*
   and it calls `freetestapi_list` with `{"entity":"product"}`.
...
```sh
export FREETESTAPI_APIKEY=sk_live_xxx            # API key
export FREETESTAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `freetestapi_list` tool
...
### Call the `freetestapi_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `freetestapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `freetestapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREETESTAPI_APIKEY` | API key sent with every request. |
| `FREETESTAPI_BASE` | Optional override of the API base URL. |
...
```sh
./freetestapi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"freetestapi_load","arguments":{"entity":"user","query":{"id":1}}}}'
```
