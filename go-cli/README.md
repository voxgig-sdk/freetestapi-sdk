...
boru-driven command-line client **and** interactive REPL for the Freetestapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/freetestapi-cli)
make build

# 2. See usage (words, entities, env vars)
./freetestapi-cli --help

# 3. Provide credentials once, via the environment
export FREETESTAPI_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./freetestapi-cli list product
./freetestapi-cli list user

# 5. Override the API base URL for a single call
FREETESTAPI_BASE=https://api.example.com ./freetestapi-cli list product

# 6. No arguments -> interactive REPL
./freetestapi-cli
freetestapi> list product
freetestapi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/freetestapi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/freetestapi-cli list product
   ```
...
Run with no arguments to open a REPL (prompt `freetestapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `product`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/freetestapi-cli`. |
...
(`./freetestapi-cli <expr>`) and interactively (the REPL), and expressions compose the
